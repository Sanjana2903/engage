...
def login_submit():
    ...
    if status == "Image not clear ! Please try again !" or status == "Data does not exist !" or status == "This user ID is not registered yet" or status == "No face detected" or status == "Multiple faces detected":
        ...
        if os.path.exists("./Attendance.csv"):
            df = pd.read_csv(os.getcwd()+"/Attendance.csv")
            out = {"ID":[str(email)],"Attendance":["Attendance Marked !"],"time":[str(dt.now())]}
            df = pd.concat([df,pd.DataFrame(out)],ignore_index=True)
            print(df)
            print(df.columns)
            df.to_csv(os.getcwd()+"/Attendance.csv")
        else:
            out = {"ID":[str(email)],"Attendance":["Attendance Marked !"],"time":[str(dt.now())]}
            df = pd.DataFrame(out)
            df.to_csv(os.getcwd()+"/Attendance.csv")
...
def register():
    ...
    if form.validate_on_submit():
        ...
        print(form.email.data,form.url.data)
        form.email.data = email
        form.url.data = url
